...
class BandsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create band" do
    assert_difference('Band.count') do
      post bands_url, params: { band: { debut_date: @band.debut_date, kind: @band.kind, members: @band.members, name: @band.name } }
    end
  ...
  test "should update band" do
    patch band_url(@band), params: { band: { debut_date: @band.debut_date, kind: @band.kind, members: @band.members, name: @band.name } }
